...
describe("combineIMessagePayloads", () => {
  ...
  it("keeps first + most recent when entry count exceeds the cap, but tracks every GUID", () => {
    const payloads = Array.from({ length: MAX_COALESCED_ENTRIES + 15 }, (_, i) =>
      makePayload({
        id: i,
        text: `msg ${i}`,
        guid: `row-${i}`,
    ...
    expect(merged.coalescedMessageGuids?.length).toBe(MAX_COALESCED_ENTRIES + 15);
    expect(merged.coalescedMessageGuids?.[0]).toBe("row-0");
    expect(merged.coalescedMessageGuids?.[MAX_COALESCED_ENTRIES + 14]).toBe("row-24");
